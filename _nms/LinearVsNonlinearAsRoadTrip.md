...
Description: 
If you are on a road trip you can estimate the time it will take to finish the trip if more distance is introduced.
If the distance and the time it took was linear, doubling the distance needed to travel would mean it would take double the time to finish the trip. This could happen if for instance the trip was one long highway.
If the distance and the time it took was non-linear, doubling the distance needed to travel would not mean the time it would take was double. There are other factors that come into play such as big cities and their traffic.
